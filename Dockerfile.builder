...
# jq extracts node kinds and field names from the grammar's
# node-types.json in the regen entrypoint below.
RUN apt-get update \
...
# gofmt formats kinds_gen.go and provides syntax check.
COPY --from=prep /usr/local/go/bin/gofmt /usr/local/bin/gofmt
...
# Node-kind and field-name constants
# Generated from the node-types.json the grammar ships next to parser.c which
# upstream generates for statically-typed bindings.
# NOTE: Relies on GNU sed's \U uppercase to produce CamelCase ids.
consts() {
  local prefix=$1 names ids dups
  names=$(cat)
  ids=$(sed -E "s/(^|_+)(.)/\U\2/g; s/^/$prefix/" <<<"$names")
  dups=$(sort <<<"$ids" | uniq -d)
  [[ -z $dups ]] || { echo "kinds_gen: colliding identifiers: $dups" >&2; return 1; }
  printf 'const (\n'
  paste <(printf '%s\n' "$ids") <(sed 's/.*/= "&"/' <<<"$names") | LC_ALL=C sort
  printf ')\n'
}

NODE_TYPES="$GRAMMAR_SRC/src/node-types.json"
# Hidden rules (leading underscore) never appear in trees and supertypes
# (entries with subtypes) are hidden by construction, so both drop out.
kinds=$(jq -r '.[] | select(.named and (has("subtypes") | not)) | .type' \
  "$NODE_TYPES" | sed '/^_/d')
fields=$(jq -r '.[] | (.fields // {}) | keys[]' "$NODE_TYPES" | sort -u)
[[ -n $kinds ]] || { echo "kinds_gen: no named node types in $NODE_TYPES" >&2; exit 1; }

{
  echo '// Code generated by bonsai-builder. DO NOT EDIT.'
  echo '//'
  echo "// Node kinds and field names from $(cat /sources/grammar.dir) $(cat /sources/grammar.version) (src/node-types.json)."
  echo
  echo "package $WASM_PACKAGE"
  echo
  ...
  if [[ -n $fields ]]; then
    echo
    echo '// FieldX constants name every field this grammar attaches to child'
    echo '// nodes, for Node.ChildByField and Node.Field.'
    consts Field <<<"$fields"
  fi
} > "$OUTPUTS/kinds_gen.go"
gofmt -w "$OUTPUTS/kinds_gen.go"
...
echo "OK: regenerated $OUTPUTS/{module_gen.go, libc_gen.go, module_gen.dat, meta_gen.go, kinds_gen.go, licenses_gen.go, LICENSE.tree-sitter, LICENSE.grammar}"
EOF
